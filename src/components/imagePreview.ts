...
function createNavButton(innerHTML: string, extraStyles: string, ownerDoc: Document): HTMLButtonElement {
  ...
  btn.classList.add('zaibar-image-viewer-button');
  btn.innerHTML = innerHTML;
  btn.style.cssText = `position:absolute;top:50%;transform:translateY(-50%);width:44px;height:44px;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:1;transition:background 0.2s;${extraStyles}`;
  return btn;
}
...
export function createImageViewer(
  ...
  registerChatSkinRoot(overlay);
  overlay.style.cssText =
    'position:fixed;inset:0;z-index:2147483647;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.72);cursor:pointer;user-select:none;';
  overlay.tabIndex = -1;

  const img = ownerDoc.createElement('img');
  img.style.cssText = 'max-width:85vw;max-height:90vh;object-fit:contain;box-shadow:0 25px 50px -12px rgba(0,0,0,.5);cursor:default;';
  overlay.appendChild(img);

  const counter = ownerDoc.createElement('div');
  counter.style.cssText = 'position:absolute;top:16px;left:50%;transform:translateX(-50%);color:white;font-size:13px;pointer-events:none;';
