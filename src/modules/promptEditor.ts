...
export class PromptEditor {
  ...
  renderList() {
    ...
    if (!this.prompts.length) {
      ...
      emptyState.className = 'rounded-xl border border-dashed border-zinc-300 p-4 text-sm text-zinc-500 dark:border-zinc-600 dark:text-zinc-400';
      emptyState.textContent = 'No custom prompts yet.';
      void (this.doc as any).l10n?.formatValue?.(getLocaleID('pref-prompteditor-empty-state')).then((value: string) => {
        if (value) {
  ...
  createPromptRow(prompt: UserPrompt) {
    ...
    row.dataset.index = String(this.prompts.findIndex((item) => item.id === prompt.id));

    const content = this.doc.createElement('div');
    content.className = 'flex items-center gap-3 overflow-hidden';

    const handle = this.doc.createElement('div');
    handle.className = 'cursor-grab text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300';
    handle.innerHTML = `
    ...
    deleteButton.className = 'opacity-0 group-hover:opacity-100 p-1 text-zinc-400 hover:text-red-500 transition-opacity focus:opacity-100';
  ...
  bindAddButton() {
    const addButtonHost = this.doc.querySelector<HTMLElement>('#prompt-add-button-host');
    ...
    if (labelElement) {
      labelElement.setAttribute('data-l10n-id', getLocaleID('pref-prompteditor-button-add'));
    }
  ...
  bindEditButtons() {
    const saveButton = this.doc.querySelector<HTMLButtonElement>('#prompt-save-btn');
    const cancelButton = this.doc.querySelector<HTMLButtonElement>('#prompt-cancel-btn');
  ...
  bindDragSort() {
    ...
    this.promptList?.addEventListener('dragstart', (event) => {
      const row = (event.target as HTMLElement | null)?.closest('[data-prompt-id]') as HTMLElement | null;
      if (!row) return;
    ...
    this.promptList?.addEventListener('dragover', (event) => {
      const row = (event.target as HTMLElement | null)?.closest('[data-prompt-id]');
      if (!row) return;
