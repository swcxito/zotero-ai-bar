 ...
 * 获取选中内容的上下文
 */
export async function getSelectionContext(reader: _ZoteroTypes.ReaderInstance<"pdf" | "epub" | "snapshot">, params: { annotation: _ZoteroTypes.Annotations.AnnotationJson; }) {
    ...
    if (itemID && index?.indexType === 'pdf' && reader._internalReader._type === 'pdf') {
        const selextedPageIndexes = isCrossPage ? [index.pageIndex!, index.pageIndex! + 1] : [index.pageIndex!];
        const fullText = await Zotero.PDFWorker.getFullText(itemID, selextedPageIndexes);
        ...
        if (lineCount <= 10) {
            ...
            if (macheCount == 1) {
                addon.data.userPrompt = getContextAroundIndex(fullText.text, [maches[0].start, maches[0].end], contextSize + 30);
                ztoolkit.log("selected context by search:", addon.data.userPrompt);
            } else if (selectedText.split(' ').length <= 5 || selectedText.length <= 5) { // words only, use position match
                // ztoolkit.log(selected.position?.rects);
                addon.data.userPrompt = getContextByPosition(selected, currentPage, contextSize, isCrossPage ? data.pages[index.pageIndex! + 1] : undefined);
...
function getContextByPosition(selected: _ZoteroTypes.Annotations.AnnotationJson, page: Array<any>, contextSize: number = 50, nextPage: Array<any> | undefined = undefined): Array<string> {
...
function parseArray(page: Array<any>, indexOffset: number = 0): Array<TextBox> {
    ...
    function walk(node: any, index: number) {
        ...
        if (Array.isArray(node)) {
            ...
            else for (let i = 0; i < node.length; i++) {
                walk(node[i], i);
            }
        }
    }
...
function countOccurrencesInFullText(fullText: string | string[], selected: string):
